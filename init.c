...
init ()
{
	...
	*/
	...
	if ((word)( blocks_allocated = allocate ( (max_blocks + 2),save_blocks )) < (word)(resident_blocks + 2))
	{
		/*
...
byte	version ;
{
	...
	if ( version == VERSION_3 )
	{
		...
		if ( enhanced )
		{
			...
			**
			**	Certain other games have debugging words "#comm", "#reco"
			**	and "#unre" which use two opcodes ($33 and $34). Their function
			**	is currently unknown.
			**
			**	These opcodes are added in the "enhanced" version only.
			*/

			((header_ptr)base_ptr) -> script_status |= VERSION_3_SOUND ;
			data_head.script_status |= VERSION_3_SOUND ;

			jmp_op2[0x33] = set_print ;
			jmp_op2[0x34] = record_mode ;
			jmp_op2[0x35] = play_sound ;

			operands[0x33] = 0 ;			/* set_print              */
			operands[0x34] = 1 ;			/* record_mode            */
			operands[0x35] = 0 ;			/* play_sound             */
		}
	}
