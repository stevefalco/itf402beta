...
Void
set_current_font ( new_font )
word	new_font ;
{
	extern word		current_font ;

	word			last_font ;

	/*
	**	When the "new_font" parameter equals 1, the normal font is selected.
	**	When the "new_font" parameter equals 3, the graphics font is selected.
	**
	**	These are currently the only valid parameters - used in "Beyond Zork".
	*/

	last_font = current_font ;
	if ( new_font != last_font )
	{
...
/*
**	Save/Restore Game Context - Global Variables.
*/

boolean		saved = FALSE ;
word		saved_stack_space[STACK_SIZE] ;
word		*saved_stack_var_ptr ;
word		*saved_stack ;
word		saved_pc_page ;
word		saved_pc_offset ;
...
save_context ()
{
	...
	extern word			saved_stack_space[] ;
	extern word			*saved_stack_var_ptr ;
	extern word			*saved_stack ;
	extern word			saved_pc_page ;
	extern word			saved_pc_offset ;
	...
	**	Stored Values:
	**
	**		-1 == 0xFFFF	: Context Save/Restore feature not implemented.
	**		0				: Context Save/Restore Error.
	**		1				: Context Save Successful.
	**		2				: Context Restore Successful.
	*/


	if ( saved_context != (byte_ptr)0 )
	{
		p = base_ptr ;
		q = saved_context ;
		for ( i = 0 ; i < data_head.save_bytes ; i++ )
			*q++ = *p++ ;
		for ( i = 0 ; i < STACK_SIZE ; i++ )
			saved_stack_space[i] = stack_space[i] ;
		saved_stack_var_ptr = stack_var_ptr ;
		saved_stack = stack ;
		saved_pc_page = pc_page ;
		saved_pc_offset = pc_offset ;
		saved = TRUE ;
		store ( (word)1 ) ;
	}
	else
	...
	else
	{
		if ( saved )
		{
			...
			for ( i = 0 ; i < data_head.save_bytes ; i++ )
				*q++ = *p++ ;
			for ( i = 0 ; i < STACK_SIZE ; i++ )
				stack_space[i] = saved_stack_space[i] ;
			stack_var_ptr = saved_stack_var_ptr ;
			stack = saved_stack ;
			pc_page = saved_pc_page ;
